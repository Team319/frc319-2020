...
public class FollowTrajectory extends CommandBase {
    ...
    private class BufferLoader implements java.lang.Runnable {
        ...
        public void run() {
            ...
            leftLastPointSent = manageBuffer(leftTalon, trajectoryToFollow.leftProfile, Robot.drivetrain.DRIVE_PROFILE,
                    leftLastPointSent);
        ...
        private int manageBuffer(BobTalonFX leftTalon, SrxMotionProfile prof, int pidfSlot, int lastPointSent) {
            if (lastPointSent >= prof.numPoints) {
                return lastPointSent;
            }

            if (!leftTalon.isMotionProfileTopLevelBufferFull() && lastPointSent < prof.numPoints) {
    ...
    public void resetTalon(BobTalonFX rightTalon2, ControlMode controlMode, double setValue) {
        // System.out.println("Clearing MP Trajectories");
        rightTalon2.clearMotionProfileTrajectories();
        rightTalon2.set(ControlMode.MotionProfile, SetValueMotionProfile.Disable.value);
